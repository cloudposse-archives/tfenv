...
__NOTE__: The utility supports a number of configuration settings which can be passed via environment variables.
  ...
  * `TFENV_PREFIX` - Prefix to prepend to all environment variables (default is `TF_VAR_`, but you could do something like `TF_VAR_app_`)
  * `TFENV_WHITELIST` - Regular expression that should match permitted environment variables (by default `.*`)
  * `TFENV_BLACKLIST` - Regular expression that should exlude unwanted/dangerous environment variables (e.g. AWS credentials)
